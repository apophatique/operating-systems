...
int main(void) {
    ...
    const HANDLE H_FILE = CreateFile(FILE_PATH, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    if (write_file_with_auto_length_counting(H_FILE, correct_data, &length_of_written_data) == 0) {
        if (send_last_error_message_to_h_error(H_STD_ERROR, &length_of_written_data) == 0) {
